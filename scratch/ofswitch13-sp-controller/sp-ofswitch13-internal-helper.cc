...
NS_LOG_COMPONENT_DEFINE ("SPOFSwitch13InternalHelper");
NS_OBJECT_ENSURE_REGISTERED (SPOFSwitch13InternalHelper);

class SPOFSwitch13Controller;

SPOFSwitch13InternalHelper::SPOFSwitch13InternalHelper ()
{
...
TypeId
SPOFSwitch13InternalHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SPOFSwitch13InternalHelper")
    .SetParent<OFSwitch13Helper> ()
    .SetGroupName ("OFSwitch13")
    .AddConstructor<SPOFSwitch13InternalHelper> ()
  ;
  return tid;
}

void 
SPOFSwitch13InternalHelper::CreateOpenFlowChannelsV2(vector<int> ctIdv){
    cout<<"Creating OpenFlow channels."<<endl;
    ...
    NetDeviceContainer switchDevices, controllerDevices;  
    Ipv4InterfaceContainer switchAddrs, controllerAddrs;
    switchDevices = m_csmaHelper.Install (m_switchNodes,  csmaChannel);
    switchAddrs = m_ipv4helper.Assign (switchDevices);

    for(uint32_t i = 0; i < ctIdv.size(); i++){
      CsmaHelper csmaH;
      csmaH.SetChannelAttribute ("DataRate", DataRateValue (DataRate ("20Mbps")));
      csmaH.SetChannelAttribute ("Delay", StringValue ("1ms"));
      //add link n0---n1
      //cout<<"m_switchNodes:"<<m_switchNodes.GetN()<<","<<m_controlNodes.GetN()<<" i: "<<i<<" ctIdv[i] "<<ctIdv[i]<<endl;
      //cout<<m_controlNodes.Get(ctIdv[i] - 1)<<endl;
      NodeContainer nc = NodeContainer(m_switchNodes.Get(ctIdv[i] - 1), m_controlNodes.Get(ctIdv[i]-1));  
      controllerDevices.Add(csmaH.Install(nc.Get(1))); 
    }
    controllerAddrs = m_ipv4helper.Assign(controllerDevices);


    for(uint32_t i = 0; i < switchAddrs.GetN(); i++){
      //cout<<switchAddrs.GetAddress (i)<<endl;
    }
    for(uint32_t i = 0; i < ctIdv.size(); i++){
      InetSocketAddress addr (switchAddrs.GetAddress (ctIdv[i]), 6653); 
      //InetSocketAddress addr (controllerAddrs.GetAddress (ctIdv[i] - 1), 6653); 
      for (OFSwitch13DeviceContainer::Iterator ofDev = m_openFlowDevs.Begin ();ofDev != m_openFlowDevs.End (); ofDev++)
      {
          cout << "Connect switch " << (*ofDev)->GetDatapathId () <<
                              " to controller " << addr.GetIpv4 () <<
                              " port " << addr.GetPort () << endl;
...
void 
SPOFSwitch13InternalHelper::CreateOpenFlowChannels(vector<int> ctIdv){
    cout<<"Creating OpenFlow channels."<<endl;
    NS_ABORT_MSG_IF (m_blocked, "OpenFlow channels already configured.");
    ...
    Ipv4InterfaceContainer switchAddrs;
    switchDevices = m_csmaHelper.Install (m_switchNodes,  csmaChannel);
    switchAddrs = m_ipv4helper.Assign (switchDevices);
...
void
SPOFSwitch13InternalHelper::CreateOpenFlowChannels (void)
{
  NS_LOG_FUNCTION (this);
  ...
  std::cout<<"m_channelType:"<<m_channelType<<std::endl;
  switch (m_channelType)
    {
    case SPOFSwitch13InternalHelper::SINGLECSMA:
      {
    ...
    case SPOFSwitch13InternalHelper::DEDICATEDCSMA:
      { 
      }
    case SPOFSwitch13InternalHelper::DEDICATEDP2P:
      {
        ...
        // segment size at SPOFSwitch13Controller and OFSwitch13Device.
            ...
            for (uint32_t ctIdx = 0; ctIdx < m_controlNodes.GetN (); ctIdx++)
              {
                ...
                m_controlApps.Get (ctIdx)->GetAttribute ("Port", portValue);
                //cout<<"PPPort: "<< portValue.Get()<<endl;
...
SPOFSwitch13InternalHelper::InstallController (
...
void
SPOFSwitch13InternalHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
NetDeviceContainer
SPOFSwitch13InternalHelper::Connect (Ptr<Node> ctrl, Ptr<Node> swtch)
{
